...
serve(async (req) => {
  ...
  try {
    ...
    // If no variants were generated (provider errors, empty content, etc.), create a safe fallback
    if (optimizedVariants.length === 0) {
      console.warn('⚠️ No optimized variants generated by providers — using baseline fallback');
      const fallbackPrompt = [
        'Improve the following prompt for clarity and actionable execution without changing its intent.',
        '',
        `Original: ${originalPrompt}`,
        '',
        'Rewrite requirements:',
        '- Clarify the objective in one sentence',
        '- Add any missing constraints (tone, length, format) as concise bullet points',
        '- Keep it neutral and implementation-agnostic',
        '- Do not include examples or answers',
        '',
        'Return only the improved prompt.'
      ].join('\n');

      const fallbackScore = evaluateOutput(fallbackPrompt, 0.25);
      optimizedVariants.push({
        prompt: fallbackPrompt,
        strategy: 'Baseline Fallback',
        score: fallbackScore,
        response: 'Optimization fallback applied',
        metrics: {
          tokens_used: fallbackPrompt.length,
          response_length: 0,
          prompt_length: originalPrompt.length,
          strategy_weight: 25,
